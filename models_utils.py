...
def read_data(dataset,num_user,similarity):
    ...
    train_files = [f for f in train_files if f.endswith(str(num_user)+'_'+str(similarity)+'.json')]
    ...
    test_files = [f for f in test_files if f.endswith(str(num_user)+'_'+str(similarity)+'.json')]
    for f in test_files:
...
def read_full_data(num_of_users,data,dataset):
    ...
    for i in range(num_of_users):
        ...
        train_data = data[2][id]
        test_data = data[3][id]
        X_train.extend(train_data['x'])
        y_train.extend(train_data['y'])
        X_test.extend(test_data['x'])
        y_test.extend(test_data['y'])
        # X_train, y_train, X_test, y_test = train_data['x'], train_data['y'], test_data['x'], test_data['y']
    if dataset == "Mnist":
        X_train = torch.Tensor(X_train).view(-1, NUM_CHANNELS, IMAGE_SIZE, IMAGE_SIZE).type(torch.float32)
        y_train = torch.Tensor(y_train).type(torch.int64)
