...
# from skiputils import averageModels
...
def Wrapper():
    ...
    # #For Diritchlet(non-iid)
    # client_dict_train = {}  # Create an empty dictionary
    # for i in range(30):
    #      with open(f'orchestra/data/30/1.0/MNIST/train/MNIST_{i}.pkl', 'rb') as f:
    #          client_dict_train.update({i:pkl.load(f)})   # Update contents of file1 to the dictionary
    # client_dict_test = {}  # Create an empty dictionary
    # for i in range(30):
    #      with open(f'orchestra/data/30/1.0/MNIST/test/MNIST_{i}.pkl', 'rb') as f:
    #          client_dict_test.update({i:pkl.load(f)})   # Update contents of file1 to the dictionary
    # for inx, client in enumerate(clients):
    #     trainset_id_list = list(client_dict_train[inx])
    #     client['mnist_trainset'] = getImage(
    #         mnist_trainset, trainset_id_list, args['batch_size'])
    #     client['mnist_testset'] = getImage(
    #         mnist_testset, list(client_dict_test[inx]), args['batch_size'])
    ...
    #     def __init__(self):
    #         super(CNN, self).__init__()
    #         self.conv1 = nn.Conv2d(1, 16, 5, 1)  # Changed 32 to 64
    #         self.conv2 = nn.Conv2d(16, 32, 5, 1)  # Adjusted to match the previous change
    #         self.fc1 = nn.Linear(4*4*32, 50)  # Adjusted to match the previous change
    ...
    #         x = Func.relu(self.conv2(x))
    #         x = Func.max_pool2d(x, 2, 2)
    #         x = x.view(-1, 4*4*32)  # Adjusted to match the previous change
    #         x = Func.relu(self.fc1(x))
    #         x = self.fc2(x)
    #         return Func.log_softmax(x, dim=1)
    # class CNN(nn.Module):
    ...
    for fed_round in range(args['rounds']):
        ...
        # y_out = h*y_out + noise
####Not done in avergaeModels so done here instead
